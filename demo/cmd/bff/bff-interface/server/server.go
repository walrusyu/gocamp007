...
func (s *Server) GetOrder(context.Context, *emptypb.Empty) (*pb.Order, error) {
...
func (s *Server) GetUser(context.Context, *emptypb.Empty) (*pb.User, error) {
	// Set up a connection to the server.
...
func (s *Server) UpdateUser(ctx context.Context, req *pb.UpdateUserRequest) (*pb.User, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := c.Update(ctx, &userpb.UpdateRequest{User: &userpb.User{
		Id:   req.User.Id,
		Name: req.User.Name,
		Age:  req.User.Age,
